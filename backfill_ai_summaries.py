...
def setup_logging():
    ...
    log_filename = f"logs/backfill_ai_summaries_{timestamp}.log"
...
def main():
    ...
    try:
        ...
        for i, article in enumerate(articles_without_summary, 1):
            try:
                logging.info(f"Processing article {i}/{total_articles}: {article.original_title}")
                ...
                if not content_to_summarize or content_to_summarize.strip() == "":
                    logging.info(f"No extracted content, extracting from URL: {article.original_link}")
                    try:
                        ...
                        logging.warning(f"Content extraction failed for {article.original_title}: {e}")
                ...
                if content_to_summarize and content_to_summarize.strip():
                    ...
                    if ai_summary and ai_summary.strip():
                        ...
                        processed_count += 1
                        logging.info(f"✓ Added AI summary for: {article.original_title}")
                    else:
                        logging.warning(f"AI summarizer returned empty result for: {article.original_title}")
                        error_count += 1
                else:
                    logging.warning(f"No content available to summarize for: {article.original_title}")
                    error_count += 1
                
                # Rate limiting - be nice to OpenAI API
                if i % 10 == 0:
                    logging.info(f"Progress: {i}/{total_articles} processed. Taking a short break...")
                    time.sleep(2)
                else:
                    time.sleep(0.5)
