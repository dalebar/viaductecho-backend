...
class TestDatabaseOperations:
    ...
    def test_insert_article_success(self, db_ops, sample_article_data):
        ...
        existing = db_ops.session.query(RSSArticle).filter_by(
            original_link=sample_article_data['original_link']
        ).first()
        if existing:
    ...
    def test_insert_article_minimal_data(self, db_ops, sample_article_data_minimal):
        ...
        # Clean up any existing test data
        existing = db_ops.session.query(RSSArticle).filter_by(
            original_link=sample_article_data_minimal['original_link']
        ).first()
        if existing:
            db_ops.session.delete(existing)
            db_ops.session.commit()
    ...
    def test_duplicate_article_prevention(self, db_ops):
        ...
        # Clean up any existing test data
        existing = db_ops.session.query(RSSArticle).filter_by(
            original_link=duplicate_data['original_link']
        ).first()
        if existing:
            db_ops.session.delete(existing)
            db_ops.session.commit()
